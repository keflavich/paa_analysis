...
if len(files) < 732:
    ...
    tbl = Eso.query_surveys(surveys='VVV', coord1=0, coord2=0, coord_sys='gal', box=5*u.deg)
...
#fits.PrimaryHDU(data=array_line, header=wcs_out.to_header()).writeto('gc_vvv_mosaic_kband.fits', overwrite=True)

hdu = fits.PrimaryHDU(data=np.zeros([10,10], dtype='float32'))
header = hdu.header
header['NAXIS1'] = shape_out[0]
header['NAXIS2'] = shape_out[0]
if not os.path.exists('gc_vvv_mosaic_kband.fits'):
    #hdu.writeto('gc_vvv_mosaic_kband.fits', overwrite=True, output_verify='fix')
    header.tofile('gc_vvv_mosaic_kband.fits', overwrite=True)
if not os.path.exists('gc_vvv_mosaic_kband_coverage.fits'):
    #hdu.writeto('gc_vvv_mosaic_kband_coverage.fits', overwrite=True, output_verify='fix')
    header.tofile('gc_vvv_mosaic_kband_coverage.fits', overwrite=True)

output_file = fits.open('gc_vvv_mosaic_kband.fits', mode='update', output_verify='fix')
if output_file[0].data.shape != shape_out:
    output_file.close()
    with open('gc_vvv_mosaic_kband.fits', 'rb+') as fobj:
        fobj.seek(len(header.tostring()) + (shape_out[0] * shape_out[1] * 4) - 1)
        fobj.write(b'\0')
    output_file = fits.open('gc_vvv_mosaic_kband.fits', mode='update', output_verify='fix')

output_coverage = fits.open('gc_vvv_mosaic_kband_coverage.fits', mode='update', output_verify='fix')
if output_coverage[0].data.shape != shape_out:
    output_coverage.close()
    with open('gc_vvv_mosaic_kband_coverage.fits', 'rb+') as fobj:
        fobj.seek(len(header.tostring()) + (shape_out[0] * shape_out[1] * 4) - 1)
        fobj.write(b'\0')
    output_coverage = fits.open('gc_vvv_mosaic_kband_coverage.fits', mode='update', output_verify='fix')
